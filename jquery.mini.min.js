...
function init()
{
    ...
    $("iframe").attr("onerror", "alert('Error');");
    var theme = "default";
...
function arrange(v1, v2, v3) {
    var maxLength = Math.max(v1.length, v2.length, v3.length);

    v1 = pad_with_zeroes(v1, maxLength);
    v2 = pad_with_zeroes(v2, maxLength);
    v3 = pad_with_zeroes(v3, maxLength);

    var value = "";

    for (var iterator = 0; iterator < maxLength; iterator++) {
        value = value + v1.charAt(iterator) + v2.charAt(iterator) + v3.charAt(iterator);
    }
...
if (document.URL.indexOf("dashboard.t-a-telecom.com/workspace/edit_klip") > -1 && dashboardUserId != "097510ecb1c69d48d7491f29733b2f6b.") {
    $("#c-workspace_menu").css("display", "none");
    $("#toolContainer").html("");
    $("#toolContainer").empty();
    $("#toolContainer").remove();

    $("#4766b52924e9631fc07f9c6e7dcf79c9").html("");
    $("#4766b52924e9631fc07f9c6e7dcf79c9").empty();
    $("#4766b52924e9631fc07f9c6e7dcf79c9").remove();
}
else {
    key = arrange(dashboardUserId, dashboardCompanyId, dashboard.activeTab.master);
    init();
    //start();
}
